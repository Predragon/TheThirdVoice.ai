...
<style>
    ...
    .copy-button { float: right; margin-top: -10px; }
</style>
...
# Initialize API key validation state
...
# Validate API key on startup if we have one
if st.session_state.gemini_api_key and not st.session_state.api_key_validated:
    st.session_state.api_key_validated = validate_api_key(st.session_state.gemini_api_key)
...
# API Key Configuration UI - Only show if no valid key
if not st.session_state.api_key_validated:
    with st.expander("🔑 Configure Google Gemini API", expanded=True):
        ...
        if st.session_state.gemini_api_key:
            st.warning("API Key loaded but not working. Please check your key or enter a new one below.")
        api_key_input = st.text_input(
            "Enter your Gemini API key:",
            value="",
            type="password",
            help="Your API key will be validated and stored temporarily in your browser session."
        )
        if st.button("Save API Key"):
            if api_key_input:
                ...
                if validate_api_key(api_key_input):
                    ...
                    st.error("Failed to validate API key. Please check your key and try again.")
                    st.session_state.api_key_validated = False
            else:
                st.warning("Please enter an API key.")
else:
    # Show a small success indicator when API key is working
    st.success("✅ API Key configured and working!")
                ...
                if not st.session_state.api_key_validated:
                    ...
                    with st.spinner("⚡ AI is analyzing your message..."):
                        ...
                        with col2:
                            ...
                            # AI-Suggested Reframe with Copy Button
                            st.markdown("#### ✨ AI-Suggested Reframe")
                            reframed = ai_coach.reframe_message(message_input, context)
                            
                            # Create columns for the reframe text and copy button
                            reframe_col1, reframe_col2 = st.columns([4, 1])
                            with reframe_col1:
                                st.markdown('<div class="ai-response">', unsafe_allow_html=True)
                                st.markdown(reframed)
                                st.markdown('</div>', unsafe_allow_html=True)
                            with reframe_col2:
                                if st.button("📋 Copy", key="copy_reframe", help="Copy reframed message"):
                                    st.write(f'<script>navigator.clipboard.writeText(`{reframed}`);</script>', unsafe_allow_html=True)
                                    st.success("Copied!", icon="✅")
                            
                            # Also show in code block for easy manual copying
        ...
        if received_message.strip():
            if not st.session_state.api_key_validated:
                ...
                with st.spinner("⚡ AI is analyzing the emotional subtext..."):
                    ...
                    # Create columns for the translation and copy button
                    ...
                    with trans_col1:
                        ...
                        st.markdown('</div>', unsafe_allow_html=True)
                    with trans_col2:
                        if st.button("📋 Copy", key="copy_translation", help="Copy emotional translation"):
                            st.write(f'<script>navigator.clipboard.writeText(`{translation}`);</script>', unsafe_allow_html=True)
                            st.success("Copied!", icon="✅")
        else:
